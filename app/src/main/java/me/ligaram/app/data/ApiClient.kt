...
object ApiClient {
    ...
    private val API_TOKEN get() = BuildConfig.API_TOKEN
    ...
    fun fetchCallInfo(number: String): ApiResult {
        return try {
            ...
            if (BuildConfig.DEBUG) Log.d("ApiClient", "→ GET $url")
            ...
            if (BuildConfig.DEBUG) Log.d("ApiClient", "← ${response.code} for $cleanNumber")

            if (response.code == 200) {
                val body = response.body.string()
                if (BuildConfig.DEBUG) Log.d("ApiClient", "Body: $body")
                if (body.isNotBlank()) {
                ...
                if (BuildConfig.DEBUG) Log.d("ApiClient", "Non-200 (${response.code}) - overlay suppressed")
                ApiResult.NoResult
            }
        } catch (e: Exception) {
            if (BuildConfig.DEBUG) Log.e("ApiClient", "Request failed: ${e.javaClass.simpleName}: ${e.message}")
