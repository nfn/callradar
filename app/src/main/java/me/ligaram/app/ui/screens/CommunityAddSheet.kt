...
private val SHEET_CLASSIFICATIONS = listOf("Perigoso", "Suspeito", "Incómodo", "Seguro", "Neutro")

private val SHEET_CLASS_ICONS = mapOf(
...
fun CommunityAddSheet(
    ...
    // Fechar teclado e autocomplete — usado em múltiplos lugares
    ...
    fun submit() {
        ...
        scope.launch {
            ...
            when (result) {
                is CommunityResult.Success -> { delay(400); onSuccess() }
                is CommunityResult.Error   -> { errorMsg = result.message; isSubmitting = false }
            }
        }
    }
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            // ── Scroll ────────────────────────────────────────────────────────
            // Fase Main: corre após os filhos (leaf→root), para o clickable da sugestão
            // processar o toque e actualizar entity antes de fechar o dropdown.
            Column(
                modifier = Modifier
                    .weight(1f)
